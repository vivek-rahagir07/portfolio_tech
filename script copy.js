...
function initAnimation(imageSource) {
    ...
    image.addEventListener('load', () => {
        // Calculate dimensions maintaining aspect ratio, max 80% of screen width
        const targetWidth = Math.min(window.innerWidth * 0.8, image.width, 800);
        const scale = targetWidth / image.width;
        const targetHeight = image.height * scale;
        
        // Center the artwork on the fullscreen canvas
        const offsetX = (window.innerWidth - targetWidth) / 2;
        const offsetY = (window.innerHeight - targetHeight) / 2;
        ...
        // Try getting image data. 
        // Note: For this to work with local files in some browsers, it requires a local server.
...
class Particle {
    constructor(x, y, size, color, isEdge) {
        ...
        if (this.isEdge) {
            ...
            // Elegant starting position: slightly scattered and above
        ...
        if (!this.isEdge) {
            ...
            if (this.revealed) {
                ...
                if (Math.abs(dx) < 0.5 && Math.abs(dy) < 0.5) {
                    ...
                    // Buttery smooth easing - no erratic jittering
                    this.x += dx * 0.08;
                    this.y += dy * 0.08;
                }
            }
...
function animate() {
    ...
    if (fillStarted) {
        fillRevealY += 4; // Reveal fill slowly from head to bottom
        for (let i = 0; i < particlesArray.length; i++) {
            const p = particlesArray[i];
            if (!p.isEdge) {
...
// Load the image directly on startup!
